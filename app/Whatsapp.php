<?php

namespace App;

use Jenssegers\Mongodb\Eloquent\Model;
class Whatsapp extends Model 
{
    public static function saveChat($data){
        $data = json_decode($data, true);
        if(isset($data['message']) && isset($data['message']['conversation'])){
            $chat = new WhatsappChat();
            $chat->text_chat = $data['message']['conversation'];
            $chat->chat_id = isset($data['key']['id']) ? $data['key']['id'] : "";
            $chat->group_id = isset($data['key']['remoteJid']) ? $data['key']['remoteJid'] : "";
            $chat->user_id = isset($data['participant']) ? $data['participant'] : "";
            $chat->chat_date = isset($data['messageTimestamp']) ? $data['messageTimestamp'] : "";
            $chat->save();

            Whatsapp::saveHastagChat($chat->text_chat, $chat->chat_id);

        }
    }

    public static function saveHastagChat($text_chat, $chat_id){
        $list_hastag = Whatsapp::getHastag($text_chat);

        foreach ($list_hastag as $hastag) {
            $hastag = strtolower($hastag);
            $hastag_chat = WhatsappChatHastag::where('chat_id', $chat_id)->where('hastag', $hastag)->first();
            if(!$hastag_chat){
                $hastag_chat = new WhatsappChatHastag();
                $hastag_chat->chat_id = $chat_id;
                $hastag_chat->hastag = $hastag;
                $hastag_chat->save();
            }
        }
    }

    public static function saveAnalytic($data){
        $analytic = new WhatsappAnalytic();
        
        $analytic->hastag = $data['hastag'];
        $analytic->total_chat = $data['total_chat'];
        $analytic->total_user = $data['total_user'];
        $analytic->total_group = $data['total_group'];
        $analytic->count_chat_7day = $data['count_chat_7day'];
        $analytic->total_user_7day = $data['total_user_7day'];
        $analytic->save();
    }

    public static function getHastag($string){
        preg_match_all("/(#\w+)/", $string, $matches);
        $hastag = array();
        if ($matches) {
            $hashtagsArray = array_count_values($matches[0]);
            $hashtags = array_keys($hashtagsArray);
        }
        return $hashtags;
    }
}